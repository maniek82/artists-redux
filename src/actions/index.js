...
export function artistListAll() {
    ...
    const request = axios.get(`${URL}/artists?_limit=6`).then(res=> {
        return res.data
    })
...
export function artistList(keywords) {
    const request = axios.get(`${URL}/artists?q=${keywords}`).then(res=> {
        return res.data
    })
...
export function artistDetail(id) {
    const request = axios.get(`${URL}/artists?id=${id}`).then(res=> {
        return res.data
    })
